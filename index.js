...
function getCarInfoByIndex(inventory, index) {
  ...
  return `This is a ${selected.car_make} ${selected.car_model}`;
}
...
function getOlderCars(inventory, maxYear) {
  const olderCars = [];
  for (let i = 0; i < inventory.length; i++) {
    if (inventory[i].car_year <= maxYear) olderCars.push(inventory[i]);
  }
  return olderCars;
}
...
function getGermanCars(inventory) {
  const germanCarMakers = ["Audi", "Mercedes-Benz", "Volkswagen", "BMW"], germanCars = [];
  for (let i = 0; i < inventory.length; i++) {
    for (let j = 0; j < germanCarMakers.length; j++) {
      if (inventory[i].car_make === germanCarMakers[j]) 
